...
__global__ void MatAddKernel(Visualization::Matrix A, Visualization::Matrix B, Visualization::Matrix C) {
    int blockRow = blockIdx.y;
    int blockCol = blockIdx.x;
    double Cvalue = 0;
    int row = threadIdx.y;
    ...
    for (int a = aBegin; a <= aEnd; a += aStep)
    {
        __shared__ double As[BLOCK_SIZE][BLOCK_SIZE];
        __shared__ double Bs[BLOCK_SIZE][BLOCK_SIZE];
        As[row][col] = A.elements[a + A.width * row + col]; //As[16*by+ty][tx] +step As[16*by+ty][tx+BLOCK_DIM] 横向延伸
        Bs[row][col] = B.elements[a + B.width * row + col];
        __syncthreads();
        for (int e = 0; e < BLOCK_SIZE; ++e) {
            Cvalue = As[row][e] + Bs[row][e]; //这里按照列循环，一列就一个Cvalue
            SetElement(C, blockRow * BLOCK_SIZE + row, blockCol * BLOCK_SIZE + col, Cvalue);
        }
        __syncthreads();
    }
    */
    ...
    if (ix < A.width && iy < A.height) {
        C.elements[iy * C.stride + ix] = A.elements[iy * A.stride + ix] + B.elements[iy * B.stride + ix];
    }
}

__global__ void MatSubKernel(Visualization::Matrix A, Visualization::Matrix B, Visualization::Matrix C) {
    int blockRow = blockIdx.y;
    int blockCol = blockIdx.x;
    double Cvalue = 0;
    int row = threadIdx.y;
...
__global__ void MatAddKernel2(Visualization::Matrix A, double B, Visualization::Matrix C) {
    int blockRow = blockIdx.y;
    int blockCol = blockIdx.x;
    double Cvalue = 0;
    int row = threadIdx.y;
    ...
    */
    ...
    if (ix < A.width && iy < A.height) {
        C.elements[iy * C.stride + ix] = A.elements[iy * A.stride + ix] + B;
    }
}

__global__ void MatSubKernel2(Visualization::Matrix A, double B, Visualization::Matrix C) {
    int blockRow = blockIdx.y;
    int blockCol = blockIdx.x;
    double Cvalue = 0;
    int row = threadIdx.y;
...
void Visualization::initEmptyMatrix(Visualization::Matrix* A, int row, int col) {
    A->width = col;
    A->height = row;
    A->stride = col;//step
    A->elements = (double*)calloc(row * col, sizeof(double));
...
void Visualization::initOnesMatrix(Visualization::Matrix* A, int row, int col) {
    A->width = col;
    A->height = row;
    A->stride = col;//step
    A->elements = (double*)calloc(row * col, sizeof(double));
...
void Visualization::matrixAdd(const Matrix A, const Matrix B, Matrix C) {
    Matrix d_A;
    d_A.width = d_A.stride = A.width;
    d_A.height = A.height;
    size_t size = A.width * A.height * sizeof(double);
...
void Visualization::matrixAdd(const Matrix A, double B, Matrix C) {
    Matrix d_A;
    d_A.width = d_A.stride = A.width;
    d_A.height = A.height;
    size_t size = A.width * A.height * sizeof(double);
    ...
    MatAddKernel2 << <dimGrid, dimBlock >> > (d_A, d_B, d_C);
    cudaMemcpy(C.elements, d_C.elements, size,
        cudaMemcpyDeviceToHost);
...
void Visualization::matrixSub(const Matrix A, const Matrix B, Matrix C) {
    Matrix d_A;
    d_A.width = d_A.stride = A.width;
    d_A.height = A.height;
    size_t size = A.width * A.height * sizeof(double);
...
void Visualization::matrixSub(const Matrix A, double B, Matrix C) {
    Matrix d_A;
    d_A.width = d_A.stride = A.width;
    d_A.height = A.height;
    size_t size = A.width * A.height * sizeof(double);
    ...
    MatSubKernel2 << <dimGrid, dimBlock >> > (d_A, d_B, d_C);
    cudaMemcpy(C.elements, d_C.elements, size,
        cudaMemcpyDeviceToHost);
